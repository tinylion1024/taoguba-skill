...
from common import (
    ...
    fetch, get_headers, retry,
    get_db, init_db,
    upsert_author, upsert_post, update_post_body, bulk_upsert_posts,
    update_crawl_log, get_last_crawl,
    load_authors_json, to_csv
)
...
def parse_views_replies(views_replies: str) -> tuple:
    vr = re.findall(r'(\d+)', views_replies)
    views = int(vr[0]) if len(vr) > 0 else 0
    replies = int(vr[1]) if len(vr) > 1 else 0
...
# 爬取单博客
# -----------------------------------------------------------------------
def crawl_blog(blog_input: str, pages: int = 3, delay: float = DEFAULT_DELAY,
               fetch_content: bool = False, save_db: bool = False,
    ...
    try:
        ...
        for i, art in enumerate(all_articles[:5], 1):
            ...
            print(f"  {i}. [{art['views']:,}阅/{art['replies']}回复] {art['publish_date']} {tag_str} {art['title'][:50]}")
...
# 批量爬取所有已知大V
# -----------------------------------------------------------------------
def crawl_all_authors(pages: int = 3, delay: float = DEFAULT_DELAY,
                      fetch_content: bool = False, save_db: bool = False,
...
def main():
    parser = argparse.ArgumentParser(
        description="Crawl blog posts from 淘股吧 (tgb.cn) big-V blogs."
    )
    ...
    parser.add_argument('--fetch-content', action='store_true',
                        help='Also fetch full article body (slow)')
    ...
    parser.add_argument('--all-authors', action='store_true',
                        help='Crawl all known authors from references/tgb_blog_authors.json')
