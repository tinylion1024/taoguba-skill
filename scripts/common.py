...
# ============ 重试装饰器 ============
def retry(max_attempts: int = 3, base_delay: float = 1.0, backoff: float = 2.0, exceptions=(Exception,)):
    """指数退避重试装饰器"""
        ...
        def wrapper(*args, **kwargs):
            for attempt in range(max_attempts):
                try:
                    ...
                    logger.warning(f"[{func.__name__}] 第{attempt + 1}次失败: {e}, {delay:.1f}s后重试...")
                    time.sleep(delay)
...
def fetch(url: str, headers: dict = None, timeout: int = 30, encoding: str = "utf-8") -> str:
    """带重试的HTTP GET"""
...
CREATE TABLE IF NOT EXISTS stock_comments (
    ...
    topic_id    TEXT NOT NULL,        -- 帖子topic_id（用于去重）
    author_name TEXT,
    author_bid  TEXT,
    content     TEXT,                 -- 评论内容
    post_url    TEXT,
...
def update_crawl_log(conn: sqlite3.Connection, crawl_type: str, target_id: str, records: int, status: str = "success"):
    conn.execute(
        """
...
def get_last_crawl(crawl_type: str, target_id: str, db_path: str = None) -> str | None:
    """获取某目标最后爬取时间（用于增量爬取）"""
    conn = get_db(db_path)
    row = conn.execute(
        """
